...
存在的意义：传统数据结构若要实现持久性结构，既访问历史版本，空间代价过高[n*h]。使用关联性的方法能更好的实现，通过计算机几何能实现h *logN。用红黑树可以使得每次的拓扑结构变换复杂度都能控制在O(1)，使得不同版本间的差距更小，每次只需要记录一次的变换，avl只能保证插入时如此，删除时不可。
...
合并做法：合并只涉及右侧，节点多分布于左侧，完全二叉结构丢失，堆结构依旧保留
